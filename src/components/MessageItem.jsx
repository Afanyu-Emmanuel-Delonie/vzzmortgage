...
const MessageItem = ({ message }) => {
  ...
  return (
    <div className={`flex flex-col ${isBot ? 'items-start' : 'items-end'} gap-1.5 group animate-slide-up`}>
      <div className={`flex items-end gap-2.5 max-w-[90%] ${isBot ? 'flex-row' : 'flex-row-reverse'}`}>
        
        {/* Avatar */}
        <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm transition-transform group-hover:scale-110 ${
          isBot ? 'bg-[#102044]/10 text-[#102044]' : 'bg-slate-200 text-slate-600'
        }`}>
